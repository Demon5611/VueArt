...
// Node.js CLI that replicates the provided bash script behavior
// - Interactive prompts (via readline)
// - API requests (via fetch)
// - JSON parsing and selection
// - Colored output
...
function extractNodeTypeId(boardResponseJson, nodeTypeName) {
  // In bash version they looked for .data.nodeTypes[].name == nodeTypeName
  const nodeTypes = boardResponseJson?.data?.nodeTypes;
  if (Array.isArray(nodeTypes)) {
    const found = nodeTypes.find((nt) => nt?.name === nodeTypeName);
    return found?.id;
  }
  // Some APIs may return created entity directly
...
async function autoCleanupDeadUnits() {
  ...
  for (const unit of items) {
    ...
    // 1) check board exists if boardId present
...
async function createUnitWithBoards(rl) {
  ...
  // Step 1: ensure unit type exists
  ...
  if (unitType === 'Stream') {
    ...
    // Project (or others) - create boards directly
    for (let i = 1; i <= boardsCount; i += 1) {
      console.log();
...
async function addBoardToExistingUnit(rl) {
  ...
  let boardId;
  if (unitType === 'Stream') {
    // Create child project unit then board
...
async function main() {
  ...
  // Dependencies: Node 18+ provides global fetch
